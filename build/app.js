...
function rangeLabelFor(s){
  if(s.isFailedRerun) return `Failed re-run · ${s.total} question${s.total===1?'':'s'}`;
...
function rangeLabelOf(o){
  return rangeLabelFor({
    isWrongPool:o.isWrongPool, isRetake:o.isRetake, isWeighted:o.isWeighted, isFailedRerun:o.isFailedRerun,
...
function normalizeWrongPool(arr){
  ...
  (Array.isArray(arr) ? arr : []).forEach(v => {
    // Accept both the current {qn, missedAt} shape and the plain-number shape
    // this pool used before date filtering existed, so nobody's saved list is
    // lost when this ships — a bare number just becomes a dated-unknown entry.
...
// Merge question ids into the persistent pool, stamped with the current time.
// Never removes — re-adding an id already on the list just refreshes its date.
...
async function removeFromWrongPool(qns){
  ...
  const kept = (state.wrongPool || []).filter(r => !drop.has(r.qn));
...
// ---------------------------------------------------------------------------
// DATE-RANGE FILTERING for the all-time missed pool (setup screen control).
// Ranges are local calendar days, not UTC, so "Today" matches what the
// candidate's own clock says today is.
// ---------------------------------------------------------------------------
const MISSED_PRESETS = [
  ['all', 'All time'],
  ['today', 'Today'],
...
function endOfLocalDay(daysAgo){
  const d = startOfLocalDay(daysAgo);
  d.setHours(23,59,59,999);
  return d;
}
// filt is {preset, from, to} where from/to are 'YYYY-MM-DD' strings from a
// <input type=date>, only meaningful when preset === 'custom'. Returns the
// [from, to] Date bounds for the selected preset, or [null, null] for "all
// time" (no bound at all — every dated entry matches, undated ones included
// by the caller separately since "all time" is the one range they DO belong to).
function missedRangeBounds(filt){
  if(filt.preset === 'today') return [startOfLocalDay(0), endOfLocalDay(0)];
  if(filt.preset === '2day') return [startOfLocalDay(1), endOfLocalDay(0)];
  if(filt.preset === '7day') return [startOfLocalDay(6), endOfLocalDay(0)];
  if(filt.preset === 'custom'){
    const from = filt.from ? new Date(filt.from + 'T00:00:00') : null;
    const to = filt.to ? new Date(filt.to + 'T23:59:59.999') : null;
    return [from, to];
  }
  return [null, null];
}
...
function wrongPoolInRange(filt){
  ...
  return state.wrongPool.filter(r => {
    if(!r.missedAt) return false;
    const t = new Date(r.missedAt).getTime();
...
function render(){
  ...
  if(state.screen === 'missed') return renderMissedList(app);
}
...
function renderSetup(app){
  ...
  if(state.wrongPool && state.wrongPool.length > 0){
    ...
    wrongPad.appendChild(presetWrap);

    if(filt.preset === 'custom'){
      wrongPad.appendChild(el('div',{class:'row', style:'margin-bottom:12px;'},[
        el('div',{},[
          el('label',{},['From']),
          el('input',{type:'date', value: filt.from, onchange:(e)=>{ state.missedFilter.from = e.target.value; render(); }})
        ]),
        el('div',{},[
          el('label',{},['To']),
          el('input',{type:'date', value: filt.to, onchange:(e)=>{ state.missedFilter.to = e.target.value; render(); }})
        ])
      ]));
    }
    ...
    wrongPad.appendChild(el('div',{class:'btn-row'},[
      el('button',{class:'btn btn-primary', disabled: qns.length===0 ? 'true' : null, onclick: async ()=>{
        const nameVal = nameInput.value.trim();
        if(!nameVal){ errDiv.textContent = 'Please enter the candidate name.'; errDiv.style.display='block'; return; }
        state.session = {
          candidateName: nameVal,
        ...
        state.screen = 'exam';
        render();
      }},[`Practice Missed Questions (${qns.length})`]),
      el('button',{class:'btn btn-outline', onclick: ()=>{ state.screen = 'missed'; render(); }},
        [`Manage Individually (${state.wrongPool.length})`]),
      el('button',{class:'btn btn-outline', onclick: async ()=>{
        if(confirm("Clear your ENTIRE all-time missed-questions list — every date, not just the selected range? This can't be undone.")){
          state.wrongPool = [];
          state.missedFilter = {preset:'all', from:'', to:''};
          await saveWrongPool([]);
          render();
        }
      }},['Clear list'])
...
function renderFinished(app){
  ...
  // pool itself, retaking a prior run's own misses, or the failed re-run
  // (also drawn entirely from the missed pool).
  const repeatMissNums = (session.isWrongPool || session.isRetake || session.isFailedRerun) ? [] :
    retakeNums.filter(qn => (session.priorMissedQns || []).includes(qn));

  if(retakeNums.length > 0 && !session.isWrongPool && !session.isRetake && !session.isFailedRerun){
...
function renderMissedList(app){
  const card = el('div',{class:'card'});
  card.appendChild(el('div',{class:'pad'},[
    el('h1',{},['Missed Questions']),
    el('div',{class:'subtitle'},['Sort, review, or individually remove questions from your all-time missed list.']),
    el('button',{class:'btn btn-outline', onclick: ()=>{ state.screen='setup'; render(); }},['← Back to menu'])
  ]));
  const pad = el('div',{class:'pad', style:'padding-top:0;'});

  if(state.wrongPool.length === 0){
    pad.appendChild(el('div',{class:'empty-note'},['Nothing on the missed list.']));
  } else {
    // Builds and starts an exam from every question currently failed (passCount
    // 0 — nothing has redeemed the last miss yet) plus every question passed
    // fewer than N times in a row since its last miss. N is re-entered each
    // time this runs rather than remembered, per how streaks keep changing.
    pad.appendChild(el('div',{class:'helper', style:'margin:0 0 4px;font-weight:700;'},
      ['Failed Questions – Run Again or Passed Less Than ___ Times']));
    pad.appendChild(el('div',{class:'row', style:'align-items:flex-end;margin-bottom:4px;'},[
      el('div',{},[
        el('label',{},['Times']),
        el('input',{type:'number', min:'0', step:'1', placeholder:'e.g. 3', value: state.failedRerunThreshold || '',
          oninput:(e)=>{ state.failedRerunThreshold = e.target.value; state.failedRerunMsg = ''; }})
      ]),
      el('button',{class:'btn btn-primary', style:'margin-bottom:16px;', onclick: async ()=>{
        const raw = (state.failedRerunThreshold || '').trim();
        const n = Number(raw);
        if(raw === '' || !Number.isInteger(n) || n < 0){
          state.failedRerunMsg = 'Enter a whole number of 0 or more.';
          render(); return;
        }
        const qns = state.wrongPool
          .filter(r => (r.passCount || 0) === 0 || (r.passCount || 0) < n)
          .map(r => r.qn);
        if(qns.length === 0){
          state.failedRerunMsg = `No questions are currently failed or passed fewer than ${n} time${n===1?'':'s'}.`;
          render(); return;
        }
        state.session = {
          candidateName: 'FailedReRun',
          secKey: 'all',
          rangeStart: null, rangeEnd: null,
          isFailedRerun: true,
          order: qns,
    ...
    if(state.failedRerunMsg){
      pad.appendChild(el('div',{class:'err', style:'margin:-2px 0 12px;'},[state.failedRerunMsg]));
    }

    if(!state.missedSort) state.missedSort = 'qn';
    pad.appendChild(el('div',{class:'helper', style:'margin:0 0 4px;font-weight:700;'},['Sort by']));
    const sortWrap = el('div',{class:'presets', style:'margin-bottom:12px;'});
    ...
    const sorted = state.wrongPool.slice().sort((a,b)=>{
      ...
      if(state.missedSort === 'passAsc') return (a.passCount||0)-(b.passCount||0) || a.qn-b.qn;
      return a.qn-b.qn;
    });
    const list = el('div',{class:'review-list'});
    sorted.forEach(r=>{
        ...
        el('div',{class:'qh'},[
          ...
          el('div',{style:'display:flex;align-items:center;gap:8px;'},[
            ...
            el('button',{class:'btn btn-danger', style:'padding:5px 12px;font-size:13px;', onclick: async ()=>{
              await removeFromWrongPool(r.qn);
              render();
            }},['Remove'])
          ])
        ])
